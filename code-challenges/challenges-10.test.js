...
const createServer = () => {
  ...
  app.get('/aboutme', (request, response) => {
    response.send('My name is Batool.');
  });
...
const count = (target, input) => {
  let countArr = input.reduce(function (accumulatorArr, valueArr, idxArr) {
    let countEl = valueArr.reduce(function (accumulatorEl, valueEl, idxEl) {
      if (valueEl === target) {
        accumulatorEl += 1;
    ...
    accumulatorArr = accumulatorArr + countEl;
