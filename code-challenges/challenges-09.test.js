...
const mapCurrentEvents = () => {
  let data = currentEvents.news.map(element => {
...
const countNumberOfElements = (arr) => {
  let count = arr.reduce((accumulator, value, index) => {
    accumulator += 1;
    return (accumulator);
  }, 0)
  return(count);
...
const returnNames = (arr) => {
  let namesArr = arr.reduce((accumulator, value, index) => {
...
const reversedString = (str) => {
  ...
  let reverseStr = strToArr.reduce((accumulator, value, index) => {
